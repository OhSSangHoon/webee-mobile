  ...
  return (
    <View className="items-center justify-center" style={{ width: size, height: size }}>
      ...
      <View className="items-center">
        <Text className="text-xl font-bold text-gray-900">{confidence.toFixed(1)}%</Text>
...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <View className={`self-start px-3 py-1.5 rounded-full ${getColors(severity).split(' ')[0]}`}>
      <Text className={`text-sm font-semibold ${getColors(severity).split(' ')[1]}`}>
        심각도: {severity}
      </Text>
    </View>
  );
...
export default function BeeDiagnosisScreen() {
  ...
  const analyzeMutation = useMutation({
    ...
    onError: (error: any) => {
      Alert.alert('오류', error.response?.data?.message || '이미지 분석에 실패했습니다');
    },
  });
  ...
  const aiMutation = useMutation({
    ...
    onError: (error: any) => {
      Alert.alert('오류', error.response?.data?.message || 'AI 분석에 실패했습니다');
    },
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await api.post('/api/v1/bee/diagnosis/save', formDataBody, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const isLoading = analyzeMutation.isPending || aiMutation.isPending;

  return (
    <SafeAreaView className="flex-1 bg-gray-50" edges={['bottom']}>
      {/* Header */}
      <View className="flex-row items-center justify-between px-4 py-3 bg-white border-b border-gray-200">
        <Pressable onPress={handleGoBack} className="w-10 h-10 items-center justify-center -ml-2">
          <Feather name="chevron-left" size={24} color="#111827" />
        </Pressable>
        <Text className="text-lg font-semibold text-gray-900">꿀벌 질병 진단</Text>
        <Pressable
          onPress={() => router.push('/diagnose-history')}
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
        ...
        {step === 'upload' && (
          <Card>
            <Text className="text-lg font-semibold text-gray-900 mb-1">꿀벌 사진 업로드</Text>
            <Text className="text-sm text-gray-500 mb-4">진단할 꿀벌 사진을 선택해주세요</Text>
            ...
            >
              {selectedImage ? (
                ...
                <View className="flex-1 items-center justify-center">
                  ...
                  <Text className="text-base font-medium text-gray-900">탭하여 이미지 선택</Text>
                  <Text className="text-sm text-gray-500 mt-1">JPG, PNG 파일 지원</Text>
                </View>
              )}
            </Pressable>

            <View className="mt-5">
              <Text className="text-sm font-medium text-gray-500 mb-2">예시 이미지</Text>
        ...
        {step === 'form' && (
          <Card>
            <Text className="text-lg font-semibold text-gray-900 mb-1">추가 정보 입력</Text>
            <Text className="text-sm text-gray-500 mb-4">정확한 진단을 위해 정보를 입력해주세요</Text>

            {/* Disease Select */}
            <View className="mb-4">
              <View className="flex-row items-center mb-1">
                <Text className="text-sm font-medium text-gray-900">질병 선택</Text>
                ...
                <View className="bg-white rounded-xl border border-gray-200 mt-1 overflow-hidden">
                  {DISEASE_OPTIONS.slice(1).map((option) => (
                    <Pressable
                      ...
                      onPress={() => {
                        setFormData((prev) => ({ ...prev, selectedDisease: option.value as BeeDiseaseType }));
            ...
            <View className="mb-4">
              <View className="flex-row items-center mb-1">
                <Text className="text-sm font-medium text-gray-900">재배 환경</Text>
                <Text className="text-sm text-red-500 ml-0.5">*</Text>
              </View>
              <View className="flex-row gap-2">
                {CULTIVATION_TYPES.map((type) => (
                  <Pressable
                    key={type.value}
                    onPress={() => setFormData((prev) => ({ ...prev, cultivationType: type.value }))}
                  ...
                  >
                    <Text
                      className={`text-sm font-medium ${
                        formData.cultivationType === type.value ? 'text-blue-600' : 'text-gray-500'
                      }`}
                    >
                      {type.label}
                    </Text>
            ...
            <View className="mb-4">
              <View className="flex-row items-center mb-1">
                <Text className="text-sm font-medium text-gray-900">재배 작물</Text>
                <Text className="text-sm text-red-500 ml-0.5">*</Text>
              </View>
              <TextInput
            ...
            {/* Location */}
            <View className="mb-4">
              <Text className="text-sm font-medium text-gray-900 mb-1">농지 위치</Text>
              <TextInput
                className="h-12 bg-gray-100 rounded-xl border border-gray-200 px-4 text-sm text-gray-900"
            ...
            {/* Additional Info */}
            <View className="mb-4">
              <Text className="text-sm font-medium text-gray-900 mb-1">추가 정보</Text>
              <TextInput
        ...
        {step === 'result' && result && (
          <>
            <Card className="mb-4">
              ...
              <Text className="text-base text-gray-600 leading-6">{result.description}</Text>
            </Card>

            <Card className="mb-4">
              <Text className="text-lg font-semibold text-gray-900 mb-3">주요 증상</Text>
              {result.symptoms.map((symptom, index) => (
                <View key={index} className="flex-row items-start mb-3">
                  <View className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-2 mr-3" />
                  <Text className="flex-1 text-base text-gray-900 leading-6">{symptom}</Text>
                </View>
              ))}
            </Card>

            <Card className="mb-4">
              <Text className="text-lg font-semibold text-gray-900 mb-3">원인</Text>
              <View className="flex-row items-start bg-yellow-50 p-4 rounded-xl">
                <Feather name="alert-circle" size={20} color="#F59E0B" />
                <Text className="flex-1 text-base text-gray-900 leading-6 ml-3">{result.cause}</Text>
              </View>
            </Card>

            <Card className="mb-4">
              <Text className="text-lg font-semibold text-gray-900 mb-3">대처 방안</Text>
              {result.solutions.map((solution, index) => (
                <View key={index} className="flex-row items-start mb-3">
                  <View className="w-5 h-5 rounded-full bg-green-100 items-center justify-center mr-3">
                    <Feather name="check" size={12} color="#10B981" />
                  </View>
                  <Text className="flex-1 text-base text-gray-900 leading-6">{solution}</Text>
                </View>
              ))}
            </Card>
            ...
            <Card>
              ...
              <Pressable onPress={handleNewDiagnosis} className="items-center mt-4">
